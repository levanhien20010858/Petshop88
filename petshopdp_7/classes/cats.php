...
$db = new Database();
$sql = "SELECT product.id,product.category_id,product.title,product.price,product.discount,product.thumbnail,product.classify_id,product.created_at,product.updated_at,product.star,product.sold,trademark.trademark_name,trademark.thumbnail_trademark FROM product,trademark WHERE classify_id = '2' AND category_id = $category_id AND product.trademark_id = trademark.id";
$kq = $db->select($sql);
?>
